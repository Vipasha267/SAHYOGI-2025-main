...
export default function NGOCaseManagement() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        const token = localStorage.getItem('token');
        // Quick fix: send as JSON, ignore file uploads for now
        const payload = { ...values, photos: [], videos: [], photoCaptions: [], videoCaptions: [] };
        // Use correct backend route for case management
        await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/casemanagement/add`, payload, {
          headers: {
